...
//      state &= (robot_->MoveTo(newPose_) == CANON_SUCCESS);
  ...
  LIBRARY_API CanonReturn Assembly::RunAssembly ()
  {
    CanonReturn rType = CANON_REJECT; //! TODO: set to CANON_RUNNING
    int counter = 0;

    do
    {

      //rType = RunAssemblyStep(counter++);
      Sleep((DWORD)(1000.0f / curFreq_));
    } while (rType == CANON_RUNNING);
  ...
  LIBRARY_API bool Assembly::applyOffset (int counter, assemblyParams &ap)
  {
    ...
    case ASSEMBLY_SPIRAL:
	  ...
	  //std::cout << "Reg spiral: x = " << deltas.x << ", y = " << deltas.y << endl;
      break;
	case ASSEMBLY_SQ_SPIRAL:
