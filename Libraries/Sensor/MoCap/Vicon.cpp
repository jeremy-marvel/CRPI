...
#include <cassert>
  ...
  #include <ctime>
  #include <windows.h>
#elif defined (__GNUC__) //Added 12/14/17 Linux compatibility - Meg
#include <unistd.h>	   // For usleep() //note - accepts microseconds not miliseconds multiply miliseconds by 1000
#else
  #include <time.h>
#endif
...
namespace Sensor
{
  ...
  void livemanVicon (void *param)
  {
    ...
    while (ka->runThread)
    {
      ...
      do
      {
		...
		usleep(100000); //microseconds, 100 miliseconds
#endif
  ...
  LIBRARY_API Vicon::Vicon (const char * ipAddress)
  {
...
#endif
#ifdef WIN32
		Sleep( 200 );  //to usleep for linux
#elif defined (__GNUC__)
		usleep(200000); // microseconds, 200 miliseconds
#endif
