...
//  Revision:        August 18, 2025
//  Author:          Medhavi Kamran, Shelly Bagchi
//
//  Description
//  ===========
...
int main()
{
    ...
    serverAddr.sin_addr.s_addr = inet_addr("169.254.152.42"); // Replace with your system IP or server ip 
    serverAddr.sin_port = htons(80); //server port 

    cout << "[DEBUG] Attempting to bind socket to 169.254.152.42" << endl;
    ...
    if (clientIP == "169.254.152.45") {  //  HoloLens IP or client ip address
    ...
    std::vector<std::pair<std::string, int>> removed_markers;// removed markers stack
   ...
   // cout << ">Creating robot using " << path.c_str() << endl;
    ...
    int grip_status = 0; // initially gripper is open 
    while (true)
    {
        cout << "started recieving." << endl;
        ...
        if (bytesReceived > 0)
        {
           ...
           // mesg.erase(std::remove(mesg.begin(), mesg.end(), '\n'), mesg.end());
           // mesg.erase(std::remove(mesg.begin(), mesg.end(), '\r'), mesg.end());

            // Get current robot pose ONCE
            robotPose curPose;
            urRobot.GetRobotPose(&curPose);

            // Convert RPY rotation vector from degrees to radians
            double rxRad = curPose.xrot * M_PI / 180.0;
            double ryRad = curPose.yrot * M_PI / 180.0;
            double rzRad = curPose.zrot * M_PI / 180.0;
            ...
            if (mag != 0) {
                // Convert RPY (in radians) to Quaternion
                ...
                std::cout << "[DEBUG] Quaternion: [" << qx << ", " << qy << ", " << qz << ", " << qw << "]" << endl;
                oss << (curPose.x) / (1000) << "," << (curPose.y) / (1000) << "," << curPose.z / 1000 << "," << qx << "," << qy << "," << qz << "," << qw << "\n";
            }
            ...
            if (mesg == "record") {
                ...
                marker_points.push_back(std::make_pair(mesg, grip_status));
                marker_indices++;

                // Send back the pose string to Unity 
                std::string response = mesg + "\n";
                send(clientSocket, response.c_str(), (int)response.length(), 0);

                cout << "[DEBUG] Recorded marker #" << marker_indices << ": " << mesg << ", grip: " << grip_status << endl;
                ...
                const int grip_status = 0;  // Update grip status to open
                ...
                cout << "[DEBUG] Gripper opened and marker recorded: " << mesg << endl;
            }
            else if (mesg == "grip_close") {
                ...
                const int grip_status = 1;  // Update grip status to closed
                ...
                cout << "[DEBUG] Gripper closed and marker recorded: " << mesg << endl;
            }
                ...
                mesg += "\n";
                int bytesSent = send(clientSocket, mesg.c_str(), (int)mesg.length(), 0);
                if (bytesSent == SOCKET_ERROR) {
                    cout << "[ERROR] Send failed: " << WSAGetLastError() << endl;
                    break;
                }
                cout << "[DEBUG] Echoed back to Unity: " << mesg;
            }
