...
//  Subsystem:       NeuralNet
//  Workfile:        RadialBasisFunctions.h
...
#ifndef RADIALBASISFUNCTIONS_H
#define RADIALBASISFUNCTIONS_H
...
namespace NeuralNet
{
  ...
  //!
  template <class T1, class T2> class LIBRARY_API RadialBasisFunction
  {
  public:
    ...
    //!
    RadialBasisFunction (int inFeat,
                         int inter1Feat,
                         int inter2Feat,
                         int outFeat,
...
//    void snapshot (NNDisplayAdapter &display);
    ...
    //! @brief Hook to set whether or not to use the new dynamic RBF functions
    //!
    //! @param useAlpha Whether or not to use the alpha-layer retraining of decision planes
    //! @param useBeta  Whether or not to use the beta-layer retraining of centered recognizers
    //! @param useGamma Whether or not to use the dynamic beta-gamma weight adjustment
    //!
    void setDynamicFunctions(bool useAlpha, bool useBeta, bool useGamma);

    //! @brief Hook to set whether the raw inputs should be used
    //!
    //! @param useRawInputs Whether or not the unscaled inputs should be used in training and testing the network
    //!
    void setUseRawInputs(bool useRawInputs);
  ...
  private:
    ...
    //! @brief The combined sum of weighted inputs into the J alpha-layer neurons
    //!
    double *hiddenJInputs_;

    //! @brief The calculated outputs leaving the J alpha-layer neurons
    //!
    double *hiddenJOutputs_;

    //! @brief The combined sum of weighted inputs into the K beta-layer neurons
    //!
    double *hiddenKInputs_;

    //! @brief The calculated outputs leaving the K beta-layer neurons
    //!
    double *hiddenKOutputs_;

    //! @brief The combined sum of weighted inputs into the output decision neuron
    //!
    ...
    //! @brief Adds a perturbation to the Gamma weights
    //!
    void addDWBetaGammaWeights ();

    //! @brief Subtracts a perturbation from the gamma weights
    //!
    void subDWBetaGammaWeights ();

    //! @brief Fill matrix dwBetaGamma_ [][] with random values in the range
    //!        -randRange_ < dw < randRange_
    //!
    //! @param val TODO
    //!
    ...
    //!
    //!
    void adjustAlphaWeight (int node);
    void adjustBetaWeights ();
