...
namespace NeuralNet
{
  template <class T1, class T2>
    RadialBasisFunction<T1, T2>::RadialBasisFunction (Logger *logger,
                                                      int inFeat,
                                                      int inter1Feat,
                                                      int inter2Feat,
                                                      int outFeat,
    ...
    randRange_(0.01)
  {
    ...
    pLogger_ = new PrivateLogger("ffn");
  ...
  template <class T1, class T2>
    void RadialBasisFunction<T1, T2>::adjustNeighborhood (int method, int size)
  {
  }
                                                     ...
                                                     const char *testPath)
  {
    ...
    if (trainPath != NULL)
    {
      ...
      if (!trainingFile)
      { 
        sprintf (message, "Cannot open training file: %s", trainPath);
        exception ("RadialBasisFunction::parseInputFile", message);
        okay -= 1;
        flag = false;
      }
      ...
      if (flag)
      {
        ...
        {
          sprintf (message, "Incorrect number of inputs in training file");
          exception ("RadialBasisFunction::parseInputFile", message);
          okay -= 1;
        }
        else
    ...
    if (testPath != NULL)
    {
      ...
      if (!testingFile)
      {
        sprintf (message, "Cannot open testing file: %s", testPath);
        exception ("RadialBasisFunction::parseInputFile", message);
        okay -= 2;
        flag = false;
      }
      ...
      if (flag)
      {
        ...
        {
          sprintf (message, "Incorrect number of inputs in testing file");
          exception ("RadialBasisFunction::parseInputFile", message);
          okay -= 2;
        }
        else
...
*/
  ...
  template <class T1, class T2> double 
    RadialBasisFunction<T1, T2>::getTrainingInput (int pattern, int input)
  {
  ...
  template <class T1, class T2> double 
    RadialBasisFunction<T1, T2>::getTrainingOutput (int pattern, int output)
  {
    if (pattern > pTrainingPatterns_ || pattern < 0 ||
  ...
  template <class T1, class T2> double
    RadialBasisFunction<T1, T2>::getRawTrainingOutput (int pattern, int output)
  {
    if (pattern > pTrainingPatterns_ || pattern < 0 ||
        output > outputFeatures_ || output < 0)
    {
  ...
  template <class T1, class T2> void RadialBasisFunction<T1, T2>::computeOutputs ()
  {
    ...
    for (j = 1; j <= interJFeatures_; ++j)
    {
      ...
      alphaFire_[j] += (hiddenJOutputs_[j] > fireThreshold) ? 1 : 0;
      alphaLife_[j] += 1;
    }
  ...
  template <class T1, class T2> double RadialBasisFunction<T1, T2>::getOutput (int index)
  {
  ...
  template <class T1, class T2> void RadialBasisFunction<T1, T2>::adjustWeights ()
  {
    ...
    double origRMSError, temp = 0.0f, popmult = 1.2f;
    int j = 0, k = 0;
    bool adjusted = false;
...
//    qqq
  ...
  template <class T1, class T2>
    void RadialBasisFunction<T1, T2>::exception (char *where, char *what)
  {
    ...
    void RadialBasisFunction<T1, T2>::adjustBetaWeights ()
  {
    ...
    if (clustering)
    {
      ...
      Clustering::kMeans kclust (inputFeatures_+1, k, NULL);
      for (i = 0; i < pTrainingPatterns_; ++i)
      {
        ...
        kclust.addTrainingPattern (input, trainingOutputs_.at(i)[1]);
      }
      ...
      for (i = 0; i < k; ++i)
      {
        ...
        input = new double[inputFeatures_ + 1];
        out = kclust.getClusterInfo (i, copy);
    ...
    for (x = 0; x < interKFeatures_; ++x)
    {
      if (x < k)
      {
	    ...
	    synapses1to2_[x][0] = 0.0f;//(double)(-interJFeatures_ + 1);
	    for (j = 1; j <= interJFeatures_; ++j)
	    { 
        if (x < k)
        {
  ...
  template <class T1, class T2>
    void RadialBasisFunction<T1, T2>::adjustGammaWeightRange ()
  {
